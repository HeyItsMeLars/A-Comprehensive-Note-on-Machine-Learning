...
\textbf{Covariance}, defined by the equation 
\begin{equation}
\frac{\sum{{(x_i-\bar{x})(y_i-\bar{y})}}{N-1}
end{equation}
Covariance is a measure of how well two variables are correlated. It looks remarkably similar to the variance equation, and that's no coincidence. A covariance that is higher in magnitude (meaning that it may be positive or negative, simply that it must be farther away from 0) indicates a stronger correlation between variables. We can see why this is by examining the numerator of the equation: the sum of $(x_i-\bar{x})(y_i-\bar{y})$ will be largest in magnitude when the sign of both factors in this expression are usually in agreement with one another over all data points. 

When this is the case, the variables are more correlated and the covariance will be positive if they are positively correlated, and negative if they are negatively correlated. If there isn't much of a correlation between the variables, the covariance will be close to 0, as each term in the summation will cancel out other terms randomly.
...
To assess the quality of a classification algorithm, we have a handful of metrics. In classification problems, we want the true negatives and positives to be as high as possible, while keeping false negatives and false positives to a minimum. Put more simply, we want make predictions that are consistently correct. To do this, we can calculate the \textbf{precision and recall scores} to assess how well our model performs.

A metric used for to ascertain how well decision tree classifiers work is called the \textbf{Gini Impurity}. It is given by the equation
\begin{equation}
G = 1-\sum_{i=1}^c{(p_i)^2}
\end{equation}
Where $p_i$ is the probability of being in a certain class. This calculation shows how well a split works at a given node. A score close or equal to 0 is very good, as this means the sum of the probabilities of being in one of the classes is high. As such, we have constructed a decision tree that has minimized the uncertainty, or equivalently, that has child nodes that are predominantly of one class. 

This concept is similar to \textbf{entropy}, given by the equation
\begin{equation}
E = -\sum_{i=1}^N{p_iln(p_i)}
\end{equation}
Both of these are metrics which can be used to discern how well we have minimized uncertainty in our models. The difference is one of how they function. In datasets with uneven class sizes, Gini Impurity may sometimes be preferred due to its sensitivity to imbalance in datasets.
